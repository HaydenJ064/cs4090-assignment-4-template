...
import pytest  # Import pytest, but it's used only within the button's action.
from tasks import load_tasks, save_tasks, filter_tasks_by_category, filter_tasks_by_priority
...
def main():
    ...
    if st.button("Run Tests"):
        ...
        result = pytest.main(["-v", "test_tasks.py"])  # Run tests in test_tasks.py
        # Display the results in the Streamlit app
        if result == 0:
            st.success("All tests passed!")
        else:
            st.error(f"Tests failed. Exit code: {result}")
